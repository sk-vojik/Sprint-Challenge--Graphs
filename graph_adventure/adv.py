...
# roomGraph = {0: [(3, 5), {'n': 1, 's': 5, 'e': 3, 'w': 7}], 1: [(3, 6), {'s': 0, 'n': 2}], 2: [(3, 7), {'s': 1}], 3: [(4, 5), {'w': 0, 'e': 4}], 4: [(5, 5), {'w': 3}], 5: [(3, 4), {'n': 0, 's': 6}], 6: [
#     (3, 3), {'n': 5, 'w': 11}], 7: [(2, 5), {'w': 8, 'e': 0}], 8: [(1, 5), {'e': 7}], 9: [(1, 4), {'n': 8, 's': 10}], 10: [(1, 3), {'n': 9, 'e': 11}], 11: [(2, 3), {'w': 10, 'e': 6}]}
...
traversalGraph = {}
directions = ('n', 's', 'e', 'w')
oppositeDirection = {'n': 's', 's': 'n', 'w': 'e', 'e': 'w'}


def traverse(player, direction=''):

    # If all the rooms have been explored then it is done.
    if len(traversalGraph.keys()) == 500:
        return
    ...
    #make connection with previous room
    if direction is not '':
        opposite = oppositeDirection[direction]
        prevRoom = player.currentRoom.getRoomInDirection(opposite)
        traversalGraph[currentRoom][opposite] = prevRoom.id

    roomExit = '?'

    # Time to go to the unexplored rooms
    for exit in player.currentRoom.getExits():
        if traversalGraph[currentRoom][exit] == '?':
            #set next direction to that exit
            roomExit = exit
            # go there, then append it to the path
            player.travel(exit)
            traversalPath.append(exit)
            # make the room we're now in the current room, and the previous rooms exit the new room
            new_room = player.currentRoom.id
            traversalGraph[currentRoom][exit] = new_room
            # Go there
            traverse(player, exit)
            break

    # Find nearest room using BFS with an unexplored exit and go there
    travelPath = []
    ...
    if roomExit is '?':
        ...
        while q.size() > 0:
            ...
            if currentRoom not in visited:
                ...
                # check if current room has unexplored exit
                if '?' in traversalGraph[currentRoom].values():
                    travelPath = path
                    q = Queue()
                    break

                #for every direction, add it to the path to search and add it to queue
                for neighbor in traversalGraph[currentRoom].values():
                    new_path = list(path)
    ...
    for r in travelPath:
        ...
        graph_keys = traversalGraph[room].keys()
    ...
    # repeat until we've explored every room
    traverse(player)
